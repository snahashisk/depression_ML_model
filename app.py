...
st.set_page_config(page_title='Depression Prediction', page_icon=':smiley:', layout='wide')
st.sidebar.title('Depression Prediction App')
st.sidebar.markdown('Enter the values below to predict depression status.')

# Define input fields
age = st.sidebar.number_input('Age', min_value=1, max_value=120, value=18)
gender = st.sidebar.selectbox('Gender', ['Male', 'Female'])
education = st.sidebar.selectbox('Education', [' School', 'Bachelor', 'Masters', 'PHD'])
BMI = st.sidebar.number_input('BMI', min_value=0, max_value=100, value=24)
Past_History_of_Depression = st.sidebar.selectbox('Past_History_of_Depression', [' Yes', 'No',])
Family_History_of_Depression = st.sidebar.selectbox('Family_History_of_Depression', [' Yes', 'No',])
Sleep_Duration_hours = st.sidebar.number_input('Sleep_Duration_hours', min_value=0, max_value=24, value=3)
Physical_Activity_minutes = st.sidebar.number_input('Physical_Activity_minutes', min_value=0, max_value=3600, value=40)
Alcohol_Consumption_drinks_per_week  = st.sidebar.selectbox('Alcohol_Consumption_drinks_per_week', ['Excessive', 'Modrate','Never'])
Smoking_Status = st.sidebar.number_input('Smoking_Status', min_value=0, max_value=100, value=20)
...
input_data = np.array([[age, gender, education,BMI ,Past_History_of_Depression,Family_History_of_Depression,Sleep_Duration_hours,Physical_Activity_minutes,Alcohol_Consumption_drinks_per_week,Smoking_Status ]],dtype='float64')
